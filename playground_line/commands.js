...
Commands.selectTool = function(toolName) {
    Inner.CallSimple("slct",toolName)
}
...
/* set 2H Pencil*/
Commands.PencilH2 = function() {
    try {
    Commands.selectTool("PbTl");
    if(Commands.hasLayer("2HPencil")) {
        Commands.selectLayer("2HPencil");
    } else {
      Commands.makeLayer();
      Commands.renameLayer("2HPencil");
    }
    Commands.moveSelectedLayerToFront();
    ...
    PS.call('slct',{
      "null" : {
        "ref" : "toolPreset",
        "name" : "KYLE Ultimate Pencil Soft"
      }
    })    
...
/* set HB Pencil*/
Commands.PencilHB = function() {
    try {
    Commands.selectTool("PbTl");
    if(Commands.hasLayer("HBPencil")) {
        Commands.selectLayer("HBPencil");
    } else {
      Commands.makeLayer();
      Commands.renameLayer("HBPencil");
    }
    Commands.moveSelectedLayerToFront();
    ...
    PS.call('slct',{
      "null" : {
        "ref" : "toolPreset",
        "name" : "Kyle's Drawing Box - Pencil 4H"
      }
    }) 
...
/* set HB Pencil*/
Commands.Pencil25 = function() {
    try {
    Commands.selectTool("PbTl");
    if(Commands.hasLayer("Pencil .25")) {
        Commands.selectLayer("Pencil .25");
    } else {
      Commands.makeLayer();
      Commands.renameLayer("Pencil .25");
    }
    Commands.moveSelectedLayerToFront();
        ...
        PS.call('slct',{
          "null" : {
            "ref" : "toolPreset",
            "name" : "Kyle's Inkbox - Comics Tech Pen SUPER FINE"
          }
        }) 
...
/* set 50 Pencil*/
Commands.Pencil50 = function() {
    try {
    Commands.selectTool("PbTl");
    if(Commands.hasLayer("Pencil .50")) {
        Commands.selectLayer("Pencil .50");
    } else {
      Commands.makeLayer();
      Commands.renameLayer("Pencil .50");
    }
    Commands.moveSelectedLayerToFront();
        ...
        PS.call('slct',{
          "null" : {
            "ref" : "toolPreset",
            "name" : "Kyle's Comics Ink - Skinny Minny"
          }
        })
...
/* set PC Pencil*/
Commands.PencilPC = function() {
    try {
    Commands.selectTool("PbTl");
    if(Commands.hasLayer("Gouche PC")) {
        Commands.selectLayer("Gouche PC");
    } else {
      Commands.makeLayer();
      Commands.renameLayer("Gouche PC");
    }
    Commands.moveSelectedLayerToFront();
    ...
    PS.call('slct',{
      "null" : {
        "ref" : "toolPreset",
        "name" : "Kyle's Paintbox Gouache A Go Go"
      }
    })
...
Commands.toggleNiceBrush = function() {
    var className = document.getElementById("item").className;
    
    if(className == "mark_off") className = "mark_on";
    else className = "mark_off"
}

/*toggle brush*/
Commands.toggleBrush = function() {
    var className = document.getElementById("brushItem").className;
...
function rt(obj) {
    ...
    PS.log(JSON.stringify(obj,null,'<\t'));
        var rtObj = PS.roundTripRef(obj);
          PS.log(JSON.stringify(rtObj,null,'>\t'));
...
rt()
rt([])
rt([{ref:"capp", enumType:"Ordn", value:"Trgt"},{property:"FrgC"}])

var ask = Commands.askObj

ask()
ask([])
ask([{ref:"capp", enumType:"Ordn", value:"Trgt"},{property:"FrgC"}])
ask(["capp",{property:"FrgC"}])
ask(["application",{property:"foregroundColor"}])
...
var myData = new Uint8Array(64);
PS.call("test",{raw:myData})
...
refEnum =   {ref:"Lyr " , enumType:"t", value:"" , property:"foo"}
refID    =  {ref:"Lyr " , id:123 , property:"foo"}
refIndex =  {ref:"Lyr " , index:123 , property:"foo"}
refName =   {ref:"Lyr " , name:"blue line" , property:"foo"}
refOffset=  {ref:"Lyr " , offset:123 , property:"foo"}


{enumerated:{<<class>>:<<value>>}}
vs
{enumerated:<<class>>, value:<<value>>}

{ref:<<class>>, enumerated:{<<enumClass>>,<<value>>}, property:<<propID>>}
{ref:<<class>>, enumType:<<enumClass>>, value:<<value>>, property:<<propID>>}
